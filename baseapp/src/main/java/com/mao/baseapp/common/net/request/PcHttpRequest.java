...
public class PcHttpRequest implements IHttpRequest{
			...
			final RequestListener callBack)
	{
		new Thread()
		{
			public void run()
			{
				try
				{
					String result = sendPost(urlStr, transMap2Params(mapParams));
					if (callBack != null)
					{
						if(result!= null){
							callBack.onRequestComplete(result);
						}else{
							callBack.onRequestError("404", "404","请求失败");
						}
					}
				} catch (Exception e)
				{
    ...
    protected String transMap2Params(Map<String, String> map) {
